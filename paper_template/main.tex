...
def make_potential(name, alpha):
	# sqrt(t^2 + alpha)
	if name == 'sqrt':
		return (lambda t: np.sqrt(t * t + alpha), lambda t: t / np.sqrt(t * t + alpha))
	# |t|^alpha, 1 < alpha <= 2
	if name == 'power':
		def dphi(t):
...
def make_problem(y, omega, beta, phi, dphi, mu):
	...
	def objective(u):
		...
		for (pr, pc), (qr, qc) in edges:
			...
			w = np.where(omega[qr, qc], 1.0, 2.0)
...
def projected_gradient(f, g, u0, maxit=80):
	u = project(u0)
	fu, gu = f(u), g(u)
	...
	for k in range(1, maxit + 1):
		if k >= 2:
			...
			eta = 1.0
		eta = float(np.clip(eta / 5.0, 1e-10, 1e10))
...
def restore(y, omega, beta=20.0, pot='sqrt', alpha=1e-4, mu0=1.0, rho=0.1, n_layers=4, maxit=80):
	...
	for _ in range(n_layers):
		...
		u, k = projected_gradient(f, g, u, maxit=maxit)
		total += k
		mu *= rho
	return u, total
